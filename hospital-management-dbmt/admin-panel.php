    ...
    if((date("Y-m-d",$appdate1)==$cur_date and date("H:i:s",$apptime1)>$cur_time) or date("Y-m-d",$appdate1)>$cur_date) {
      $check_query = mysqli_query($con,"select apptime from appointmenttb where doctor='$doctor' and appdate='$appdate' and apptime='$apptime'");
        ...
        if(mysqli_num_rows($check_query)==0){
          $query=mysqli_query($con,"insert into appointmenttb(pid,fname,lname,gender,email,contact,doctor,docFees,appdate,apptime,userStatus,doctorStatus,reason,admin_id) 
          values('$pid','$fname','$lname','$gender','$email','$contact','$doctor','$docFees','$appdate','$apptime','1','1','$reason','1')");
          $q2=mysqli_query($con,"select ID from appointmenttb ORDER BY ID DESC");
          if($q2){
...
<script>
    ...
    document.getElementById('reason').addEventListener('input', function() {
        ...
        console.log("Reason to visit:", reason);
    });
</script>
